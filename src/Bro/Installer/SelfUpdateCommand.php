...
class SelfUpdateCommand extends Command
{
    ...
    protected function initialize(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->remoteInstallerFile = 'http://orkestro.pro/symfony-bro/installer';
    ...
    private function installerIsUpdated()
    {
        ...
        if (false === $remoteVersion = @file_get_contents('http://orkestro.pro/symfony-bro/bro.version')) {
            throw new \RuntimeException('The new version information of the Bro Installer couldn\'t be downloaded from the server.');
        }

        if ($localVersion === $remoteVersion) {
            $this->output->writeln('<info>Bro Installer is already up to date.</info>');
            $isUpdated = true;
        } else {
            $this->output->writeln(sprintf('// <info>updating</info> Bro Installer to <comment>%s</comment> version', $remoteVersion));
        }
    ...
    private function downloadNewVersion()
    {
        ...
        if (!is_writable($this->currentInstallerFile)) {
            throw new \RuntimeException('Bro Installer update failed: the "'.$this->currentInstallerFile.'" file could not be written');
        }

        if (!is_writable($this->tempDir)) {
            throw new \RuntimeException('Bro Installer update failed: the "'.$this->tempDir.'" directory used to download files temporarily could not be written');
        }

        if (false === $newInstaller = @file_get_contents($this->remoteInstallerFile)) {
            throw new \RuntimeException('The new version of the Bro Installer couldn\'t be downloaded from the server.');
        }
    ...
    private function rollback()
    {
        $this->output->writeln(array(
            ...
            'The previous Bro Installer version has been restored.',
            '',
        ));
